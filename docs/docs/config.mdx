...
| app:altnumbertoterminal <VersionBadge version="v0.15" />            | bool     | Set to true to send Alt+number and Alt+Q through Alt+P key combinations to the focused terminal instead of app shortcuts (defaults to false, Windows only)                                                                                                  |
...
| term:ctrlclickmovescursor <VersionBadge version="v0.15" /> | bool     | when enabled, Ctrl+click in a terminal sends left/right arrow keys to move the shell cursor within the current input row (default false)                                                                                                                       |
...
For reference, this is the current default configuration (v0.15.0):
...
```json
{
    ...
    "term:osc52": "always",
    "term:ctrlclickmovescursor": false,
    "term:cursor": "block",
